    ...
    for model in models:
        for m in model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight)  # Updated to newer version with underscore
...
class ResEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + residual  # Use addition instead of += to avoid inplace operation
        out = self.relu(out)
        return out
...
class SpatialAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.gamma * weights + x  # Use addition instead of += to avoid inplace operation
        return out
...
class AffinityAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        sab = self.sab(x)
        cab = self.cab(x)
        out = sab + cab  # Use addition instead of += to avoid inplace operation
        return out
...
# Add wrapper class to interface with training pipeline
class CSNetModel:
    """
    CSNet model wrapper with configuration interface compatible with training pipeline.
    """
    
    @staticmethod
    def get_default_config(image_size):
        """
        Get default configuration for CSNet model.
        ...
        return {
            ...
            "spatial_dims": 2,  # Not used by CSNet, but needed for interface
            "channels": [32, 64, 128, 256, 512],  # Not directly used but included for consistency
        }
    ...
    def __init__(
        ...
        **kwargs                  # Accept other parameters for interface compatibility
        ...
        """
        Initialize the CSNet model with configurable parameters.
        
        Args:
            in_channels: Number of input channels
            out_channels: Number of output channels
            **kwargs: Additional arguments (not used by CSNet but accepted for interface compatibility)
        """
    ...
    def build(self):
        """
        Build the CSNet model with the specified configuration.
        
        Returns:
            A configured CSNet model instance
        """
        model = CSNet(
